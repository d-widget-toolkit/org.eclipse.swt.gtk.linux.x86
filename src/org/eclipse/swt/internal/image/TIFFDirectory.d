...
void decodePixels(ImageData image)  {
    ...
    int destIndex = 0;
    int length = to!int(stripOffsets.length);
...
int formatStrips(int rowByteSize, int nbrRows, byte[] data, int maxStripByteSize, int offsetPostIFD, int extraBytes, int[][] strips) {
    ...
    */
    ...
    if (rowByteSize > maxStripByteSize) {
        ...
        n = to!int(data.length / rowByteSize);
        nbrRowsPerStrip = 1;
    } else {
        int nbr = to!int(data.length + maxStripByteSize - 1) / maxStripByteSize;
    ...
    */
    ...
    int mod = to!int(data.length % stripByteSize);
...
int[] formatColorMap(RGB[] rgbs) {
    ...
    */
    ...
    int offsetGreen = to!int(rgbs.length);
    int offsetBlue = to!int(rgbs.length * 2);
...
void write(int photometricInterpretation)  {
    ...
    int cnt = to!int(stripOffsets.length);
    if (cnt > 1) {
    ...
    if (isColorMap) writeEntry(TAG_ColorMap, TYPE_SHORT, to!int(colorMap.length), colorMapOffset);
