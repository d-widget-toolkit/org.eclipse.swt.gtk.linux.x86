...
void drawHighlight(GC gc, int rightEdge) {
    ...
    if(gradients is null)
        return;
    int gradientsSize = to!int(gradients.length);
    ...
    int d = to!int(parent.tabHeight - parent.topCurveHighlightEnd.length /2);
...
int preferredWidth(GC gc, bool isSelected, bool minimum) {
    ...
    if (minimum) {
        ...
        if (text !is null && toString32(text).length > minChars) {
            if (useEllipses()) {
                int end = to!int(minChars < ELLIPSIS.length + 1 ? minChars : minChars - ELLIPSIS.length);
